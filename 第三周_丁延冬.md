...
class Solution {
    ...
    public String[] permutation(String s) {
        ...
        // 3. 核心代码
        backtrack(vals,track);
        // 4. 返回值
        int size = res.size();
        String[] _res = new String[size];
        for(int i = 0; i <size; i++){
            _res[i] = res.get(i);
        }
        return _res;
    }
    public void backtrack(char[] vals, LinkedList<Character> track ){
        // 触发结束条件
        if(track.size() == vals.length){
            // 将当前的track转换为String
            // String string = new String(track);
            StringBuilder string = new StringBuilder();
            for(int i = 0; i < track.size(); i++){
                string.append(track.get(i));
            }
