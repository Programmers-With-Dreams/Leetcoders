...
class Solution {
    public String minNumber(int[] nums) {
        ...
        while(!_nums.isEmpty()){
            ...
            res.concat(Integer.toString(next));
            _nums.remove(new Integer(next));
            if(!_nums.isEmpty())
                next = _nums.get(0);
        }
        // 5. 返回值
    ...
    public boolean compare(int a, int b){
        ...
        int times = _b.length();
        if(_a.length()>_b.length()) times = _a.length();
        int _a_ = 0, _b_ = 0;// 双指针
        // 核心代码
        for(int i = 0; i < times; i++){
            if(_a_ >= _a.length()-1) _a_ = _a.length()-1;
            if(_b_ >= _b.length()-1) _b_ = _b.length()-1;
            if(_a.charAt(_a_)>_b.charAt(_b_)){
                return false;
            }
            _a_++;
            _b_++;
